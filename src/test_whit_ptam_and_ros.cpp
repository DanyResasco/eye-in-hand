...
Camera::Camera(): it(nh)
{
	...
	// img_converter.imageCb()
	// image_sub_ = nh.subscribe("/usb_cam/image_raw/camera/image_raw", 1, &Camera::imageCb, this);
    image_pub_ = it.advertise("/camera/output_video", 1);
	
	sub = it.subscribe("/usb_cam/image_raw", 1, &Camera::imageCb, this);
